...
def build_default_specs(target_freq: str = "W-FRI", z_win: int = 52) -> Dict[str, LayerSpec]:
    ...
    liq_spec = LayerSpec(
        name="Liquidity",
        target_freq="W-FRI",            # fijo semanal para que el z no se rompa
        default_fill="ffill",
        series=[
            SeriesSpec(fred_id="EFFR", alias="EFFR", units="pp_from_percent",
                       freq_objetivo="D", agg="last"),
            SeriesSpec(fred_id="NFCI", alias="NFCI", units="level",
                       freq_objetivo="W-FRI", agg="last"),   # <- vuelve a llamarse NFCI
        ],
        derived=[
